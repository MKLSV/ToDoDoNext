...
bot.on('callback_query', async (callbackQuery) => {
        ...
        await bot.sendMessage(chatId, 'Выберите выполненную задачу:', {
            reply_markup: {
                inline_keyboard: tasks.map(task => [{
                    ...
                    // callback_data: `complete_${task._id.toString()}` // Используем ID вместо текста
                }])
            }
        });
    }
    ...
    if (data.startsWith('complete_')) {
        ...
        // saveTodo({ ...taskToUpdate, isCompleted: true, completedAt: Date.now() })
        try {
            // await axios.put(`https://to-do-do-next-red.vercel.app/api/tasks/${updatedTask._id}`, updatedTask);
            await todoService.save({ ...taskToUpdate, isCompleted: true, completedAt: Date.now() })
        } catch (e) {
            console.log(e)
            await bot.answerCallbackQuery(callbackQuery.id);
...
async function saveTask(taskData) {
    ...
    try {
        ...
        //     console.log('ok')
        await todoService.save(dataToSend)
    } catch (e) {
        console.log(e)
    }
